...
class SubscriberRoleTests(unittest.TestCase):
    def setUp(self):
        ...
        (TradeControl.get_instance()).add_products("eytan", self.__store.get_name(),
                                                   [{"name": "eytan",
                                                     "price": 12,
    ...
    def test_logout(self):
        ...
        # All valid
        self.assertTrue(self.__subscriber.logout(user_nickname)['response'])
        self.assertTrue((TradeControl.get_instance()).get_curr_user().is_logged_out())

        # Invalid - user already logged out
        self.assertFalse(self.__subscriber.logout(user_nickname)['response'])
    ...
    def test_open_store(self):
        ...
        # All valid
        self.assertTrue(self.__subscriber.open_store(user_nickname, "myFirstStore")['response'])
        ...
        self.assertFalse(self.__subscriber.open_store(user_nickname, "myFirstStore")['response'])
        ...
        self.assertFalse(self.__subscriber.open_store(bad_user.get_nickname(), "not myFirstStore")['response'])
        self.assertEqual(stores_num, len((TradeControl.get_instance()).get_stores()))
        ...
        self.assertFalse(self.__subscriber.open_store(user_nickname, "          ")['response'])
        self.assertEqual(stores_num, len((TradeControl.get_instance()).get_stores()))
        self.assertIsNone((TradeControl.get_instance()).get_store("          "))

    def test_view_personal_purchase_history(self):
        # Empty purchases
        self.assertListEqual([], self.__subscriber.view_personal_purchase_history("eytan", flag_for_tests=True)['response'])
        ...
        # Not empty
        ls = self.__subscriber.view_personal_purchase_history("eytan", flag_for_tests=True)['response']
        self.assertEqual(1, len(self.__subscriber.view_personal_purchase_history("eytan", flag_for_tests=True)['response']))
        ls = self.__subscriber.view_personal_purchase_history("eytan", flag_for_tests=True)['response']
        purchases_lst = [jsonpickle.decode(e).get_products() for e in
                         self.__subscriber.view_personal_purchase_history("eytan", flag_for_tests=True)['response']]
