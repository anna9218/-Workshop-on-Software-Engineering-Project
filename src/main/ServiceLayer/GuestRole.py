...
class GuestRole:
    ...
    def register(self, nickname, password):
        if Security.get_instance().validated_password(password) and TradeControl.get_instance().validateNickName(nickname):
    ...
    def display_stores_info(self, store, store_info_flag, products_flag):
        if store_info_flag:
            return TradeControl.get_instance().get_store(store.get_name()).get_info()
        else:
            if products_flag:
                return TradeControl.get_instance().get_store(store.get_name()).get_inventory()
